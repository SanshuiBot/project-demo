...
各 demo 入口统一为各自目录下的 `index.html`；首页跳转全部使用相对路径，部署到任意子路径（如 `/project-demo/`）都能正确工作：
...
- ♿ **细节考究**：窄屏适配、性能优化（后台自动暂停渲染、脚本后置不阻塞首屏、音频按需加载等）；动画全量开放，不作系统减弱动态降级——demo 的核心展示就是这些动画。
...
### 玩法
- **拖拽** 旋转视角，**滚轮** 缩放距离，**右键拖拽** 平移视角；
- 底部按钮切换布局：`表格排列` / `球面排列` / `螺旋排列` / `网格排列`；
- 鼠标悬停卡片查看元素中英文名，**双击卡片** 弹出详细介绍（原子序数、质量、族/周期、元素分类、所在区与一句话简介）；
- 详情支持关闭按钮 / Esc 键 / 点击遮罩三种方式关闭。
